...
function ReadingRow({ chapter, paramId }: Props) {
  ...
  const { setSideBar, sideBar, showNavbar, setShowNavbar, setCurrentChapter, setCurrentVerse, currentVerse, setSideBarVerse, sideBarVerse, chapterVoiseSegments } = useContext(dispatcherconst)
  const [allVerses, setAllVerses] = useState<IPage[]>([]);
  const pageNumberRef = useRef<number>(chapter.pages[0]);
  const prevNumberRef = useRef<number>(0);
  const currentAudio = useRef<any>()
  const scrollfromTop = useRef<any>()
  const currentPlayingAudio = useRef<number>(0)
  const [activeWords, setActiveWords] = useState<number>(0)
  const [activeVerse, setActiveVerse] = useState<any>(0)
  ...
  // to pass chapter to SideBar
  const getChapterVerses = async (pageNumber: number = chapter.pages[0]) => {
    const response = await fetch(`${baseUrl}/verses/by_page/${pageNumber}?word_fields=text_uthmani_simple,code_v1&words=true`);
    const { verses } = await response.json();
    setAllVerses((prevAllVerses) => [...prevAllVerses, ...verses.filter((verse: any) => verse.verse_key.split(":")[0] === paramId)]);
  };

  const getChapterVoiceSegments = async () => {
    const { audio_files } = await chapterVoiseSegments(7, chapter.id, true);
    console.log(audio_files)
    setChapterAudio(audio_files[0])
    setVirsesTiming(audio_files[0].verse_timings);
  }


  useEffect(() => {
    const ele = document.getElementById(`${currentVerse}`);
    if (ele) {
      ele.scrollIntoView({
        behavior: 'smooth',
      ...
      const listElement = document.querySelector('.List'); // Assuming List is a class
      // if (listElement) {
      //   listElement.scrollBy(0, -75);
      // } else {
      //   console.log('Element with class "List" not found.');
      // }
    } else {
      console.log(`Element with ID ${currentVerse} not found.`);
    }
  ...
  useEffect(() => {
    ...
    getChapterVoiceSegments()
  }, []);
  const getStartingVerse = () => {
    if (startingVerse) {
      const ele = document.getElementById(`${startingVerse}`);
      ele?.scrollIntoView({
        behavior: "auto",
        block: "start",
        inline: "start"
      ...
      if (listElement) {
        listElement.scrollBy(0, -75);
        listRef.current.scrollToRow(100)
      } else {
        console.log('Element with class "List" not found.');
      }
    }
  ...
  useEffect(() => {
    getStartingVerse()
    // listRef.current?.scrollToRow(startingVerse);
  }, [startingVerse])



  const startAudio = (chapter?: number, verse?: number, word?: number, wordAudio?: string) => {
    if (currentAudio.current.src != chapterAudio?.audio_url && wordAudio) {
      console.log(currentAudio.current.duration)
      currentAudio.current.src = 'https://verses.quran.com/' + wordAudio;
    ...
    if (chapter && verse && word) {
      // console.log(verse,word, virsesTiming[verse].segments[word])
      currentAudio.current.currentTime = virsesTiming[verse - 1].segments[word][1] / 1000
      currentAudio.current.play();
    }
    if (verse && !word) {
      currentAudio.current.currentTime = virsesTiming[verse - 1].timestamp_from / 1000
      currentAudio.current.play();
    }
    currentAudio.current.play();
    getCurrentWord()
  ...
  const getCurrentWord = () => {
    ...
    const currentTime = currentAudio?.current?.currentTime * 1000;
    virsesTiming.map((verseData: any) => {
  ...
  useEffect(() => {
    currentAudio.current?.addEventListener('timeupdate', getCurrentWord)
    return () => {
      currentAudio.current?.addEventListener('timeupdate', getCurrentWord)
    }
  ...
  return (
    <div className={` transition-all duration-300  absolute right-0 h-dvh ml-0 w-full  ${sideBar ? 'md:w-[calc(100%-20em)] md:ml-[20em]' : 'w-full'}`}>
      <audio ref={currentAudio} />
      <div className={`fixed ${showNavbar ? 'top-16' : 'top-0'} flex  border-b-2 items-center  transition-[top]  duration-[0.4s] ease-in-out  left-0 w-[calc(100%-10px)] z-[999] bg-white text-black`}>
        <button className="text-[.9rem] font-[700] text-gray-700 ml-5 " onClick={() => setSideBar(!sideBar)}>{chapter.name_simple}</button>
        <div className={`${headerSwitcher ? 'mx-auto ' : 'hidden'}`}>
          <div className="switcher w-40   flex p-[3px] text-sm  rounded-3xl space-x-3 bg-slate-100">
            <div className=" flex-1 bg-white py-1 rounded-3xl flex justify-center items-center space-x-2">
              <span><svg viewBox="0 0 24 24" fill="none" width={13} xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M7 22C5.93913 22 4.92178 21.5785 4.17163 20.8284C3.42149 20.0782 3 19.0609 3 18C3 16.9391 3.42149 15.9217 4.17163 15.1715C4.92178 14.4214 5.93913 14 7 14" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path d="M7 2C5.93913 2 4.92178 2.42136 4.17163 3.17151C3.42149 3.92165 3 4.93913 3 6V18" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path d="M7 2H19C19.5304 2 20.0392 2.21074 20.4142 2.58582C20.7893 2.96089 21 3.46957 21 4V16" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path d="M7 14H19C19.5304 14 20.0392 14.2107 20.4142 14.5858C20.7893 14.9609 21 15.4696 21 16V20C21 20.5304 20.7893 21.0391 20.4142 21.4142C20.0392 21.7893 19.5304 22 19 22H7" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> </g></svg></span>

            </div>
            <div className=" flex-1 bg-white py-1 rounded-3xl flex justify-center items-center space-x-2">
              <span><svg viewBox="0 0 24 24" fill="none" width={13} xmlns="http://www.w3.org/2000/svg" stroke="#000000"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fillRule="evenodd" clipRule="evenodd" d="M10.9436 1.25H13.0564C14.8942 1.24998 16.3498 1.24997 17.489 1.40314C18.6614 1.56076 19.6104 1.89288 20.3588 2.64124C21.1071 3.38961 21.4392 4.33856 21.5969 5.51098C21.75 6.65019 21.75 8.10583 21.75 9.94359V14.0564C21.75 15.8942 21.75 17.3498 21.5969 18.489C21.4392 19.6614 21.1071 20.6104 20.3588 21.3588C19.6104 22.1071 18.6614 22.4392 17.489 22.5969C16.3498 22.75 14.8942 22.75 13.0564 22.75H10.9436C9.10583 22.75 7.65019 22.75 6.51098 22.5969C5.33856 22.4392 4.38961 22.1071 3.64124 21.3588C2.89288 20.6104 2.56076 19.6614 2.40314 18.489C2.24997 17.3498 2.24998 15.8942 2.25 14.0564V9.94358C2.24998 8.10582 2.24997 6.65019 2.40314 5.51098C2.56076 4.33856 2.89288 3.38961 3.64124 2.64124C4.38961 1.89288 5.33856 1.56076 6.51098 1.40314C7.65019 1.24997 9.10582 1.24998 10.9436 1.25ZM6.71085 2.88976C5.70476 3.02502 5.12511 3.27869 4.7019 3.7019C4.27869 4.12511 4.02502 4.70476 3.88976 5.71085C3.75159 6.73851 3.75 8.09318 3.75 10V14C3.75 15.9068 3.75159 17.2615 3.88976 18.2892C4.02502 19.2952 4.27869 19.8749 4.7019 20.2981C5.12511 20.7213 5.70476 20.975 6.71085 21.1102C7.73851 21.2484 9.09318 21.25 11 21.25H13C14.9068 21.25 16.2615 21.2484 17.2892 21.1102C18.2952 20.975 18.8749 20.7213 19.2981 20.2981C19.7213 19.8749 19.975 19.2952 20.1102 18.2892C20.2484 17.2615 20.25 15.9068 20.25 14V10C20.25 8.09318 20.2484 6.73851 20.1102 5.71085C19.975 4.70476 19.7213 4.12511 19.2981 3.7019C18.8749 3.27869 18.2952 3.02502 17.2892 2.88976C16.2615 2.75159 14.9068 2.75 13 2.75H11C9.09318 2.75 7.73851 2.75159 6.71085 2.88976ZM7.25 8C7.25 7.58579 7.58579 7.25 8 7.25H16C16.4142 7.25 16.75 7.58579 16.75 8C16.75 8.41421 16.4142 8.75 16 8.75H8C7.58579 8.75 7.25 8.41421 7.25 8ZM7.25 12C7.25 11.5858 7.58579 11.25 8 11.25H16C16.4142 11.25 16.75 11.5858 16.75 12C16.75 12.4142 16.4142 12.75 16 12.75H8C7.58579 12.75 7.25 12.4142 7.25 12ZM7.25 16C7.25 15.5858 7.58579 15.25 8 15.25H13C13.4142 15.25 13.75 15.5858 13.75 16C13.75 16.4142 13.4142 16.75 13 16.75H8C7.58579 16.75 7.25 16.4142 7.25 16Z" fill="#000000"></path> </g></svg></span>
      ...
      <AutoSizer onResize={() => cache.current?.clearAll()}>
        {({ height, width }) => (
          <List
            className="List"
            height={height}
            rowCount={allVerses.length + 1}
            rowHeight={({ index }) => cache.current?.rowHeight({ index }) || 400}
            deferredMeasurementCache={cache.current!}
            width={width}
            ref={listRef}
            onScroll={({ scrollTop }) => {
              (scrollTop > 120 ? setHeaderSwitcher(true) : setHeaderSwitcher(false));

              scrollfromTop.current = scrollTop
              const windowHeight = height;
              const scrollBottom = scrollTop + windowHeight;
              // console.log(scrollBottom < prevScrollPos || scrollBottom === 0)
              setShowNavbar(scrollBottom < prevScrollPos || scrollBottom === 0);
              prevScrollPos = scrollBottom;

              if (pageNumberRef.current <= chapter.pages[1] && pageNumberRef.current !== prevNumberRef.current) {
                getChapterVerses(pageNumberRef.current);
                prevNumberRef.current = pageNumberRef.current;
                pageNumberRef.current++;
              }
            }}
            // to get the current played audio
            onRowsRendered={({ startIndex, stopIndex }) => {
              // Get the ID of the first item in view
              console.log(startIndex, stopIndex,)
              const rowHeight = cache.current?.getHeight(startIndex, 0) || 400;


              // Calculate the range of visible items
              // const start:any = Math.floor(scrollfromTop.current / rowHeight);
              // const end = Math.min(start + Math.ceil(height / rowHeight), allVerses.length);
              const firstVisibleVerseId = allVerses[startIndex - 1]?.verse_number;

              setSideBarVerse(firstVisibleVerseId);
            }}
            ...
            rowRenderer={({ key, index, style, parent }) => {
              ...
              if (index === 0) {
                return (
                  <CellMeasurer
                    ...
                    rowHeight={400}
                  >
                    <div style={style} className="mb-10">

                      {/* switcher */}
                      <div className={`header flex justify-center mb-16`}>
                        <div className="switcher w-80  flex p-[3px] text-sm  rounded-3xl space-x-3 bg-slate-100">
                          <div className=" flex-1 bg-white py-2 rounded-3xl flex justify-center items-center space-x-2">
                            <span><svg viewBox="0 0 24 24" fill="none" width={15} xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path d="M7 22C5.93913 22 4.92178 21.5785 4.17163 20.8284C3.42149 20.0782 3 19.0609 3 18C3 16.9391 3.42149 15.9217 4.17163 15.1715C4.92178 14.4214 5.93913 14 7 14" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path d="M7 2C5.93913 2 4.92178 2.42136 4.17163 3.17151C3.42149 3.92165 3 4.93913 3 6V18" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path d="M7 2H19C19.5304 2 20.0392 2.21074 20.4142 2.58582C20.7893 2.96089 21 3.46957 21 4V16" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> <path d="M7 14H19C19.5304 14 20.0392 14.2107 20.4142 14.5858C20.7893 14.9609 21 15.4696 21 16V20C21 20.5304 20.7893 21.0391 20.4142 21.4142C20.0392 21.7893 19.5304 22 19 22H7" stroke="#000000" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"></path> </g></svg></span>
                            <span>Transilation</span>
                          </div>
                          <div className=" flex-1 bg-white py-2 rounded-3xl flex justify-center items-center space-x-2">
                            <span><svg viewBox="0 0 24 24" fill="none" width={15} xmlns="http://www.w3.org/2000/svg" stroke="#000000"><g id="SVGRepo_bgCarrier" strokeWidth="0"></g><g id="SVGRepo_tracerCarrier" strokeLinecap="round" strokeLinejoin="round"></g><g id="SVGRepo_iconCarrier"> <path fillRule="evenodd" clipRule="evenodd" d="M10.9436 1.25H13.0564C14.8942 1.24998 16.3498 1.24997 17.489 1.40314C18.6614 1.56076 19.6104 1.89288 20.3588 2.64124C21.1071 3.38961 21.4392 4.33856 21.5969 5.51098C21.75 6.65019 21.75 8.10583 21.75 9.94359V14.0564C21.75 15.8942 21.75 17.3498 21.5969 18.489C21.4392 19.6614 21.1071 20.6104 20.3588 21.3588C19.6104 22.1071 18.6614 22.4392 17.489 22.5969C16.3498 22.75 14.8942 22.75 13.0564 22.75H10.9436C9.10583 22.75 7.65019 22.75 6.51098 22.5969C5.33856 22.4392 4.38961 22.1071 3.64124 21.3588C2.89288 20.6104 2.56076 19.6614 2.40314 18.489C2.24997 17.3498 2.24998 15.8942 2.25 14.0564V9.94358C2.24998 8.10582 2.24997 6.65019 2.40314 5.51098C2.56076 4.33856 2.89288 3.38961 3.64124 2.64124C4.38961 1.89288 5.33856 1.56076 6.51098 1.40314C7.65019 1.24997 9.10582 1.24998 10.9436 1.25ZM6.71085 2.88976C5.70476 3.02502 5.12511 3.27869 4.7019 3.7019C4.27869 4.12511 4.02502 4.70476 3.88976 5.71085C3.75159 6.73851 3.75 8.09318 3.75 10V14C3.75 15.9068 3.75159 17.2615 3.88976 18.2892C4.02502 19.2952 4.27869 19.8749 4.7019 20.2981C5.12511 20.7213 5.70476 20.975 6.71085 21.1102C7.73851 21.2484 9.09318 21.25 11 21.25H13C14.9068 21.25 16.2615 21.2484 17.2892 21.1102C18.2952 20.975 18.8749 20.7213 19.2981 20.2981C19.7213 19.8749 19.975 19.2952 20.1102 18.2892C20.2484 17.2615 20.25 15.9068 20.25 14V10C20.25 8.09318 20.2484 6.73851 20.1102 5.71085C19.975 4.70476 19.7213 4.12511 19.2981 3.7019C18.8749 3.27869 18.2952 3.02502 17.2892 2.88976C16.2615 2.75159 14.9068 2.75 13 2.75H11C9.09318 2.75 7.73851 2.75159 6.71085 2.88976ZM7.25 8C7.25 7.58579 7.58579 7.25 8 7.25H16C16.4142 7.25 16.75 7.58579 16.75 8C16.75 8.41421 16.4142 8.75 16 8.75H8C7.58579 8.75 7.25 8.41421 7.25 8ZM7.25 12C7.25 11.5858 7.58579 11.25 8 11.25H16C16.4142 11.25 16.75 11.5858 16.75 12C16.75 12.4142 16.4142 12.75 16 12.75H8C7.58579 12.75 7.25 12.4142 7.25 12ZM7.25 16C7.25 15.5858 7.58579 15.25 8 15.25H13C13.4142 15.25 13.75 15.5858 13.75 16C13.75 16.4142 13.4142 16.75 13 16.75H8C7.58579 16.75 7.25 16.4142 7.25 16Z" fill="#000000"></path> </g></svg></span>
                            <span>Reading</span>
                          </div>
                        </div>
                        {/* to show audio use attr controls */}
                      </div>


                      {/* besmallah */}
                      <div className="pre_Bismallah flex items-center space-y-5 flex-col">

                        <div className="text-4xl font-thuluth_ ">
                          <span>سورة</span>
              ...
              return (
                <CellMeasurer
                  ...
                  rowIndex={index}
                >
                  <div style={{ ...style }} id={`${index}`} data-size={`${cache.current.getHeight(index, 0)}`}>
                    <div className={`translt_item flex flex-col   justify-between ${index !== allVerses.length ? 'border-b-[1px]' : ""} 
                  // this to handel  the background for the current verse playes
                  ${currentVerse && currentVerse == index ? 'bg-gray-50' : ''}
                  `}>
                      {allVerses[index - 1] && allVerses[index - 1].words ? (
                        <div className={`flex ${showNavbar ? 'py-2' : ''}`}>
                          {/* icons */}
                          <div className="icnos   mt-12 mr-20 ">
                            <ul className=" space-y-1">
                              <li className="notes group  p-2 rounded-full hover:bg-teal-100 w-fit">
                                <svg
                                  ...
                                  <g id="SVGRepo_tracerCarrier"
                                    strokeLinecap="round"
                                    strokeLinejoin="round" strokeWidth="4.8">

                                    <path
                                      className="stroke-slate-800 group-hover:stroke-teal-400"
                                      opacity="0.5" d="M3 10C3 6.22876 3 4.34315 4.17157 3.17157C5.34315 2 7.22876 2 11 2H13C16.7712 2 18.6569 2 19.8284 3.17157C21 4.34315 21 6.22876 21 10V14C21 17.7712 21 19.6569 19.8284 20.8284C18.6569 22 16.7712 22 13 22H11C7.22876 22 5.34315 22 4.17157 20.8284C3 19.6569 3 17.7712 3 14V10Z" stroke="#000000" strokeWidth="2.5">
                                    </path>
                                  </g>
                                  <g id="SVGRepo_iconCarrier">
                                    <path className="stroke-slate-700 group-hover:stroke-teal-400" d="M8 12H16" strokeWidth="1.5" ></path>
                                    <path className="stroke-slate-700 group-hover:stroke-teal-400" d="M8 8H16" strokeWidth="1.5" ></path>
                                    <path className="stroke-slate-700 group-hover:stroke-teal-400" d="M8 16H13" strokeWidth="1.5" ></path>
                                  </g>
                                </svg>
                              </li>


                              <li className="play group  p-2 rounded-full hover:bg-teal-100 w-fit" onClick={() => {
                                startAudio(chapter.id, allVerses[index - 1].verse_number)


                              }} >
                                <svg viewBox="0 0 24 24" fill="none" width={18} height={18} xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_iconCarrier"> <path className="stroke-slate-500 group-hover:stroke-teal-400" d="M16.6582 9.28638C18.098 10.1862 18.8178 10.6361 19.0647 11.2122C19.2803 11.7152 19.2803 12.2847 19.0647 12.7878C18.8178 13.3638 18.098 13.8137 16.6582 14.7136L9.896 18.94C8.29805 19.9387 7.49907 20.4381 6.83973 20.385C6.26501 20.3388 5.73818 20.0469 5.3944 19.584C5 19.053 5 18.1108 5 16.2264V7.77357C5 5.88919 5 4.94701 5.3944 4.41598C5.73818 3.9531 6.26501 3.66111 6.83973 3.6149C7.49907 3.5619 8.29805 4.06126 9.896 5.05998L16.6582 9.28638Z" strokeWidth="2" ></path> </g></svg>
                              </li>
                              <li className="tafseris group  p-2 rounded-full hover:bg-teal-100 w-fit">
                                <svg viewBox="0 0 24 24" fill="none" width={18} height={18} xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_iconCarrier"> <path className="stroke-slate-400 group-hover:stroke-teal-400" d="M20.082 3.01787L20.1081 3.76741L20.082 3.01787ZM16.5 3.48757L16.2849 2.76907V2.76907L16.5 3.48757ZM13.6738 4.80287L13.2982 4.15375L13.2982 4.15375L13.6738 4.80287ZM3.9824 3.07501L3.93639 3.8236L3.9824 3.07501ZM7 3.48757L7.19136 2.76239V2.76239L7 3.48757ZM10.2823 4.87558L9.93167 5.5386L10.2823 4.87558ZM13.6276 20.0694L13.9804 20.7312L13.6276 20.0694ZM17 18.6335L16.8086 17.9083H16.8086L17 18.6335ZM19.9851 18.2229L20.032 18.9715L19.9851 18.2229ZM10.3724 20.0694L10.0196 20.7312H10.0196L10.3724 20.0694ZM7 18.6335L7.19136 17.9083H7.19136L7 18.6335ZM4.01486 18.2229L3.96804 18.9715H3.96804L4.01486 18.2229ZM2.75 16.1437V4.99792H1.25V16.1437H2.75ZM22.75 16.1437V4.93332H21.25V16.1437H22.75ZM20.0559 2.26832C18.9175 2.30798 17.4296 2.42639 16.2849 2.76907L16.7151 4.20606C17.6643 3.92191 18.9892 3.80639 20.1081 3.76741L20.0559 2.26832ZM16.2849 2.76907C15.2899 3.06696 14.1706 3.6488 13.2982 4.15375L14.0495 5.452C14.9 4.95981 15.8949 4.45161 16.7151 4.20606L16.2849 2.76907ZM3.93639 3.8236C4.90238 3.88297 5.99643 3.99842 6.80864 4.21274L7.19136 2.76239C6.23055 2.50885 5.01517 2.38707 4.02841 2.32642L3.93639 3.8236ZM6.80864 4.21274C7.77076 4.46663 8.95486 5.02208 9.93167 5.5386L10.6328 4.21257C9.63736 3.68618 8.32766 3.06224 7.19136 2.76239L6.80864 4.21274ZM13.9804 20.7312C14.9714 20.2029 16.1988 19.6206 17.1914 19.3587L16.8086 17.9083C15.6383 18.2171 14.2827 18.8702 13.2748 19.4075L13.9804 20.7312ZM17.1914 19.3587C17.9943 19.1468 19.0732 19.0314 20.032 18.9715L19.9383 17.4744C18.9582 17.5357 17.7591 17.6575 16.8086 17.9083L17.1914 19.3587ZM10.7252 19.4075C9.71727 18.8702 8.3617 18.2171 7.19136 17.9083L6.80864 19.3587C7.8012 19.6206 9.0286 20.2029 10.0196 20.7312L10.7252 19.4075ZM7.19136 17.9083C6.24092 17.6575 5.04176 17.5357 4.06168 17.4744L3.96804 18.9715C4.9268 19.0314 6.00566 19.1468 6.80864 19.3587L7.19136 17.9083ZM21.25 16.1437C21.25 16.8295 20.6817 17.4279 19.9383 17.4744L20.032 18.9715C21.5062 18.8793 22.75 17.6799 22.75 16.1437H21.25ZM22.75 4.93332C22.75 3.47001 21.5847 2.21507 20.0559 2.26832L20.1081 3.76741C20.7229 3.746 21.25 4.25173 21.25 4.93332H22.75ZM1.25 16.1437C1.25 17.6799 2.49378 18.8793 3.96804 18.9715L4.06168 17.4744C3.31831 17.4279 2.75 16.8295 2.75 16.1437H1.25ZM13.2748 19.4075C12.4825 19.8299 11.5175 19.8299 10.7252 19.4075L10.0196 20.7312C11.2529 21.3886 12.7471 21.3886 13.9804 20.7312L13.2748 19.4075ZM13.2982 4.15375C12.4801 4.62721 11.4617 4.65083 10.6328 4.21257L9.93167 5.5386C11.2239 6.22189 12.791 6.18037 14.0495 5.452L13.2982 4.15375ZM2.75 4.99792C2.75 4.30074 3.30243 3.78463 3.93639 3.8236L4.02841 2.32642C2.47017 2.23065 1.25 3.49877 1.25 4.99792H2.75Z" fill="#000000"></path> <path className="stroke-slate-700 group-hover:stroke-teal-400" d="M12 5.50049V20.5005" stroke="#000000" strokeWidth="1.5"></path> </g></svg>
                              </li>
                              <li className="reflections group  p-2 rounded-full hover:bg-teal-100 w-fit">
                                <svg viewBox="0 0 24 24" fill="none" width={18} height={18} xmlns="http://www.w3.org/2000/svg"><g id="SVGRepo_iconCarrier"> <path className="stroke-slate-500 group-hover:stroke-teal-400" d="M21.0039 12C21.0039 16.9706 16.9745 21 12.0039 21C9.9675 21 3.00463 21 3.00463 21C3.00463 21 4.56382 17.2561 3.93982 16.0008C3.34076 14.7956 3.00391 13.4372 3.00391 12C3.00391 7.02944 7.03334 3 12.0039 3C16.9745 3 21.0039 7.02944 21.0039 12Z" stroke="#000000" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"></path> </g></svg>
                              </li>
                              <li className="more group  p-2 rounded-full hover:bg-teal-100 w-fit">
                                <svg viewBox="0 0 24 24" className="stroke-slate-400 group-hover:stroke-teal-400" width={18} height={18} xmlns="http://www.w3.org/2000/svg"><path className="stroke-slate-400 group-hover:stroke-teal-400" d="M8,12a2,2,0,1,1-2-2A2,2,0,0,1,8,12Zm10-2a2,2,0,1,0,2,2A2,2,0,0,0,18,10Zm-6,0a2,2,0,1,0,2,2A2,2,0,0,0,12,10Z" id="Horizontal"></path> </svg>
                              </li>
                            </ul>
                          </div>
                          {/* translated items */}
                          <div className="h-fit  w-full" >
                            <div style={{ direction: "rtl" }} className={` my-12 pt-2 text-[7vh] sm:text-[5vh]  font-[100]  code_v1-${allVerses[index - 1].page_number}`}>
                              {allVerses[index - 1].words.map((word: any, wordIndex: number) => (
                                // words
                                word.audio_url ? <span data-word={`${chapter.id}-${allVerses[index - 1].verse_number}-${word.position}`} className={`_Word cursor-pointer  hover:text-teal-400 mx-[.14rem] text-gray-800 ${currentVerse === allVerses[index - 1].verse_number && wordIndex + 1 == activeWords ? 'text-primary' : ''}`} key={word.id} onClick={() => startAudio(chapter.id, allVerses[index - 1].verse_number, wordIndex, word.audio_url)}>{word.code_v1} </span>
                                  : <span className="_Word hover:text-teal-400 mx-[.14rem] text-gray-800" key={word.id} >{word.code_v1} </span>
                              ))}
                            </div>
                            <div id={`translation-${index}`} className="transilation mb-16">
                              {allVerses[index - 1].words.map((word: any, wordIndex: number) => (
                                <span key={word.id}>{word.translation ? word.translation.text : ""} </span>
                              ))}
                            </div>
                          </div>
